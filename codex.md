...
There is exactly one owner-configured administrator. Implemented capabilities are one-time setup, login/logout, dashboard reads, and username/password changes requiring the current password. Future UI capabilities: create/edit pages; save drafts; publish/unpublish; delete/move/rename; manage navigation and assets/files; inspect version history/diffs; restore revisions; manage redirects and site settings. Every privileged request must verify the sole administrator, including future content APIs and assets. A hidden SPA route is not authorization. Cloudflare Access is not provisioned.

`/admin` lazy-loads the bilingual React admin UI and CSS; `/admin/account` provides account changes. The Worker serves an empty shell with no draft/account data, and each admin API verifies its own access. `GET /api/admin/setup` exposes only two booleans, `initialized` and `setupAvailable`. Setup/login require an exact same-origin request; authenticated changes also verify a session-bound CSRF token. JSON requests are bounded to 4 KiB. Cookie ambiguity is rejected, errors omit storage/credential details, and dynamic admin responses are no-store and noindex.
...
Future modules: content editor and management UI, file lifecycle, visual navigation management, redirect management, settings and deliberate caching. Keep storage in Worker-side repositories with portable domain contracts. ContentService remains internal with no HTTP mutation routes and must receive authenticated administrator requests before any HTTP exposure.
